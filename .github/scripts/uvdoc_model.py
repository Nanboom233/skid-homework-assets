...
# Vendored UVDoc network definition. Keep upstream URLs in asset-sources.json.
...
class ResnetStraight(nn.Module):
    def __init__(
        ...
        self.stride = stride or [1, 1, 2, 2]
        self.relu = nn.ReLU(inplace=True)
        self.block_nums = block_nums or [3, 4, 6, 3]
...
class UVDocnet(nn.Module):
    ...
    def forward(self, x):
        ...
        bridge_concat = torch.cat(
            [bridge_1, bridge_2, bridge_3, bridge_4, bridge_5, bridge_6],
            dim=1,
        )
